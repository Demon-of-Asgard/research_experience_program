...
for hi in os.listdir(pth):
    ...
    if 'png' not in hi:
        if 'jpg' not in hi:
            dtn = hi
            dtn = "Lk_0_Hk_0_lmu_10_hmu_150_Lmu_30_ar_0.7_deg_90.0.npy"
            ...
            for i in MUT:
                LMUT.append(1/i)
            km = -150#/float(mu)
            sfn = "./f_insta_map/insta_map_bi_Lmu_"+Lmu+"_ar_"+ar+"_deg_"+deg+"_rgmuk_"+str(max(MUT))+"_"+str(min(MUT))+"_"+str(abs(km))+"_tick.jpg"
            """print((sfn))   
            print(exists(sfn))
            print((nfn))
            print(exists(nfn))"""
