...
def fim_eig(z,modk):
    ...
    for i in range(len(LHSM)):
        for j in range(len(LHSM[i])):
            if i == j:
                if i%4 ==0:
                    LHSM[i][j] = LHSM[i][j]+(-Omg+omg+vxp*modk)+Lda(vxp,vxn,vz,fmu(z))*(2*eps2)+lda0(fmu(z))
                if i%4 == 1:
                    LHSM[i][j] = LHSM[i][j]+((-Omg+omg+vxn*modk)+Lda(vxp,vxn,vz,fmu(z))*(2*eps1)+lda0(fmu(z)))#*(1-0.0001)
                if i%4 == 2:
                    LHSM[i][j] = LHSM[i][j]+(-Omg+omga+vxp*modk)+Lda(vxp,vxn,vz,fmu(z))*(2*eps2)+lda0(fmu(z))
                if i%4 == 3:
                    LHSM[i][j] = LHSM[i][j]+((-Omg+omga+vxn*modk)+Lda(vxp,vxn,vz,fmu(z))*(2*eps1)+lda0(fmu(z)))#*(1-0.00001)
        j =  i   #see test part
        if i%4 ==0:
            if j+1 < 4:
                LHSM[i][j+1] = LHSM[i][j+1]-hvv(vxp,vxn,fmu(z))*eps1
            if j+3 < 4:
                LHSM[i][j+3] = LHSM[i][j+3]-hvva(vxp,vxn,fmu(z))*eps1
        if i%4 == 2:
            if j-1 > -1:
                LHSM[i][j-1] = LHSM[i][j-1]-hvv(vxp,vxn,fmu(z))*eps1
            if j+1 < 4:
                LHSM[i][j+1] = LHSM[i][j+1]-hvva(vxp,vxn,fmu(z))*eps1
        if i%4 == 1:
            if j-1 > -1:
                LHSM[i][j-1] = LHSM[i][j-1]-hvv(vxp,vxn,fmu(z))*eps2
            if j+1 < 4:
                LHSM[i][j+1] = LHSM[i][j+1]-hvva(vxp,vxn,fmu(z))*eps2
        if i%4 == 3:
            if j-1 > -1:
                LHSM[i][j-1] = LHSM[i][j-1]-hvva(vxp,vxn,fmu(z))*eps2
            if j-3 > -1:    
                LHSM[i][j-3] = LHSM[i][j-3]-hvv(vxp,vxn,fmu(z))*eps2
    NCFM = LHSM
...
for g in AR:
    ...
    for h in LLmu:
        ...
        fn = "/hetghome/jordan/Cose_nu/research_experience_program/COSEnu/f_insta_map/Lk_"+str(int(Lmodk/(10**(4))))+"_Hk_"+str(int(Hmodk/(10**(4))))+"_lmu_"+str(lmu)+"_hmu_"+str(hmu)+"_Lmu_"+str(Lmu)+"_ar_"+str(ar)+"_deg_"+str(deg)+"_gz_od2"        
        fnc = fn+'_f'
